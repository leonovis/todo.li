...
import { Note, InputEnum } from "../screens/Index";
...
const NoteCard = ({ note, onUpdate }: NoteCardProps) => {
    ...
    const [inputData, setInputData] = useState<Partial<Note>>(note);
    ...
    return (
        <div key={note.id} className="h-48 group relative rounded-md flex flex-col justify-between shadow-slate-900 shadow-md p-4 bg-gradient-to-r from-slate-800 to-slate-700">
        <div>
          <input className={
            clsx(inputClasses,
